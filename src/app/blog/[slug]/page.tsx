...
export async function generateMetadata({ params }: DetailedBlogPageProps): Promise<Metadata> {
...
export default async function DetailedBlogPage({ params }: DetailedBlogPageProps) {
    ...
    return (
        <Container maxWidth='xl'>
            <Box
                sx={{ display: 'flex', marginTop: '20px', flexDirection: { xs: 'column', md: 'row' }, gap: { md: '20px' } }}
            >
                    ...
                    >
                        ...
                        {blog.description.html ? (
                            <Box
                                sx={{
                                    lineHeight: '1.8',
                                    '& h1, & h2, & h3, & h4, & h5, & h6': { marginTop: '20px', marginBottom: '10px', fontWeight: 'bold' },
                                    '& p': { marginBottom: '15px' },
                                    '& ul, & ol': { marginBottom: '15px', paddingLeft: '20px' },
                                    '& li': { marginBottom: '5px' },
                                    '& blockquote': { borderLeft: '4px solid #ccc', paddingLeft: '15px', marginBottom: '15px', fontStyle: 'italic' },
                                    '& code': { backgroundColor: 'rgba(255, 255, 255, 0.1)', padding: '2px 4px', borderRadius: '4px', fontSize: '0.9em' },
                                    '& pre': { backgroundColor: 'rgba(255, 255, 255, 0.1)', padding: '15px', borderRadius: '8px', overflow: 'auto', marginBottom: '15px' },
                                }}
                                dangerouslySetInnerHTML={{ __html: blog.description.html }}
                            />
                        ) : (
                            <Typography sx={{ lineHeight: '1.8' }}>{blog.description.text}</Typography>
                        )}
                    </Box>
                </Box>
